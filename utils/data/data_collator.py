...
class DataCollator:
    ...
    def decoder_call(self, batch, return_tensors):
        ...
        for instance in batch:
            ...
            tokenized_input = self.tokenizer(task_input, add_special_tokens=False,return_tensors=return_tensors)["input_ids"][0][-self.max_prompt_len:]
            tokenized_label = self.tokenizer(label, add_special_tokens=False, return_tensors=return_tensors)["input_ids"][0][:self.max_ans_len]
            ...
            if not self.inference:
                ...
                sources.append(self.tokenizer.decode(tokenized_input, skip_special_tokens=False, clean_up_tokenization_spaces=False)
                               + self.tokenizer.decode(tokenized_label, skip_special_tokens=False, clean_up_tokenization_spaces=False))
            else:
